...
1. `PROJECT_STATE.md` —— 当前可用状态、进度、里程碑、协作协议、回归风险。
2. `WORKFLOW.md` —— 至少读 §1 核心流程；若是多人/多机器协作，再读协作拓扑章节。
3. 你的角色规则：执行者读 `EXECUTOR_RULES.md`；规划者读 `PLANNER_RULES.md`。
4. `PLANNER_EXCHANGE.md` —— 协调板：任务认领、资源归属、决策变更、预警、待拍板。动手前扫一眼，别和别人撞。
5. 你接到的 `plans/<编号>-*.md`，以及 `LESSONS.md` 里匹配工种标签的章节 + §DEBUG。不要整本经验库硬塞进上下文。
